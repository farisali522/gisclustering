...
from .models import DapilProv, CalegProv, RekapSuaraPilegProv, DetailSuaraPilegProv, RekapSuaraPilegProvKab, RekapSuaraPilegProvDapil

@admin.register(RekapSuaraPilegProvKab)
class RekapSuaraPilegProvKabAdmin(admin.ModelAdmin):
    list_display = ('wilayah_kab', 'info_tps_dpt_kab')
    list_filter = ('dapil_prov',)
        ...
        for p in partais:
            ...
            def _gv_kab(obj, pid=p_id, c=p_color):
                if not hasattr(obj, '_party_votes'):
                    ...
                    qs = DetailSuaraPilegProv.objects.filter(kecamatan__kab_kota=obj).values('partai_id').annotate(total=Sum('jumlah_suara'))
    ...
    def wilayah_kab(self, obj):
        ...
        url = reverse('admin:pileg_prov_rekapsuarapilegprov_changelist')
...
@admin.register(RekapSuaraPilegProvDapil)
class RekapSuaraPilegProvDapilAdmin(admin.ModelAdmin):
    ...
    def get_list_display(self, request):
        ...
        for p in partais:
            ...
            def _gv_dapil(obj, pid=p_id, c=p_color):
                if not hasattr(obj, '_party_votes'):
                    from django.db.models import Sum
                    qs = DetailSuaraPilegProv.objects.filter(kecamatan__kab_kota__dapil_prov=obj).values('partai_id').annotate(total=Sum('jumlah_suara'))
                    obj._party_votes = {item['partai_id']: item['total'] for item in qs}
                    obj._max_vote = max(obj._party_votes.values()) if obj._party_votes else -1
    ...
    def wilayah_dapil(self, obj):
        ...
        url = reverse('admin:pileg_prov_rekapsuarapilegprovkab_changelist')
        full_url = f"{url}?dapil_prov__id__exact={obj.id}"
...
@admin.register(DapilProv)
class DapilProvAdmin(admin.ModelAdmin):
...
class CalegProvResource(resources.ModelResource):
    dapil = fields.Field(column_name='dapil', attribute='dapil', widget=ForeignKeyWidget(DapilProv, 'nama_dapil'))
...
@admin.register(CalegProv)
class CalegProvAdmin(ImportExportModelAdmin):
    resource_class = CalegProvResource
    formats = [XLSX]
...
class RekapSuaraPilegProvResource(resources.ModelResource):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            ...
            for p in partais:
                ...
                # 2. Untuk DPRD Provinsi, format template selalu sediakan 12 Caleg per Partai secara fix
                for i in range(1, 13):
    ...
    def export_field(self, field, obj, **kwargs):
        ...
        if field.attribute and field.attribute.startswith('P_'):
            ...
            ds = DetailSuaraPilegProv.objects.filter(kecamatan=obj.kecamatan, partai__nama_partai__iexact=nama_p, caleg__isnull=True).first()
            ...
            if len(parts) == 2 and parts[1].isdigit():
                ...
                ds = DetailSuaraPilegProv.objects.filter(kecamatan=obj.kecamatan, partai__nama_partai__iexact=nama_p, caleg__no_urut=no_urut).first()
                return ds.jumlah_suara if ds else 0
    ...
    def after_import_row(self, row, row_result, **kwargs):
        ...
        try:
            dapil_kec = kec.kab_kota.dapil_prov.first()
        except Exception:
            dapil_kec = None
        ...
        for key, val in row.items():
            ...
            if k_str.startswith('P_'):
                ...
                if prt:
                    DetailSuaraPilegProv.objects.update_or_create(
            ...
            # Format C_NAMA_NO (Misal: C_GERINDRA_1)
                ...
                if len(parts) == 2 and parts[1].isdigit():
                    ...
                    if prt:
                        ...
                        if dapil_kec:
                            clg = CalegProv.objects.filter(partai=prt, no_urut=no_urut, dapil=dapil_kec).first()
                        else:
                            clg = None
                        if clg:
                            DetailSuaraPilegProv.objects.update_or_create(
...
class RekapSuaraPilegProvForm(forms.ModelForm):
    class Meta:
        model = RekapSuaraPilegProv
        fields = ['kecamatan', 'total_suara_tidak_sah']
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            dapil_kec = obj.kecamatan.kab_kota.dapil_prov.first()
        ...
        script_hitung = """
        <style>
        #rekapsuarapilegprov_form, #content-main { font-family: 'Segoe UI', system-ui, -apple-system, BlinkMacSystemFont, 'Roboto', 'Helvetica Neue', sans-serif !important; }
        ...
        self.fields['info_dp'] = forms.CharField(label="Dapil Provinsi", initial=nama_dapil, required=False, disabled=True, widget=forms.TextInput(attrs={'class': 'readonly'}))
        ...
        if filter_p:
            from django.urls import reverse
            reset_url = reverse('admin:pileg_prov_rekapsuarapilegprov_change', args=[obj.pk])
        ...
        # === PREFETCH DATA SUARA ===
        p_ids = {d.partai_id: d.jumlah_suara for d in DetailSuaraPilegProv.objects.filter(kecamatan=obj.kecamatan, caleg__isnull=True)}
        c_ids = {d.caleg_id: d.jumlah_suara for d in DetailSuaraPilegProv.objects.filter(kecamatan=obj.kecamatan, caleg__isnull=False)}

        caleg_by_partai = defaultdict(list)
        if dapil_kec:
            for c in CalegProv.objects.filter(dapil=dapil_kec).select_related('partai').order_by('no_urut'):
        ...
        for p in partais:
            ...
            for c in caleg_by_partai.get(p.id, []):
                ...
                f_c = f'su_c_{c.id}'
                self.fields[f_c] = forms.IntegerField(label=f"\u2514 {c.no_urut}. {c.nama_caleg}", initial=val_c, required=False, min_value=0, help_text=format_html('<span style="color:#666;">{} dari Suara Sah</span>', pct_c), widget=forms.NumberInput(attrs={'class': 'input-paslon'}))
                order[f_c] = self.fields.pop(f_c)

        self.fields = order


@admin.register(RekapSuaraPilegProv)
class RekapSuaraPilegProvAdmin(ImportExportModelAdmin):
    form = RekapSuaraPilegProvForm
    resource_class = RekapSuaraPilegProvResource
    formats = [XLSX]
    ...
    def get_list_display(self, request):
        ...
        def _gen_wilayah(obj, bq=base_query):
            from django.urls import reverse
            url = reverse('admin:pileg_prov_rekapsuarapilegprov_change', args=[obj.pk])
        ...
        for p in partais:
            ...
            def _gv(obj, pid=p_id, c=p_color, bq=p_query, qc=p_conn):
                ...
                if not hasattr(obj, '_party_votes'):
                    from django.db.models import Sum
                    qs = DetailSuaraPilegProv.objects.filter(kecamatan=obj.kecamatan).values('partai_id').annotate(total=Sum('jumlah_suara'))
                    obj._party_votes = {item['partai_id']: item['total'] for item in qs}
                    obj._max_vote = max(obj._party_votes.values()) if obj._party_votes else -1
    ...
    def save_model(self, request, obj, form, change):
        ...
        for n, v in form.cleaned_data.items():
            if n.startswith('su_p_'):
                ...
                clg = CalegProv.objects.get(id=int(n[5:]))
                DetailSuaraPilegProv.objects.update_or_create(
    ...
    def get_fields(self, request, obj=None):
        if obj and obj.pk:
            dummy = RekapSuaraPilegProvForm(instance=obj, request=request)
            return list(dummy.fields.keys())
        return ['kecamatan', 'total_suara_tidak_sah']

    def get_form(self, request, obj=None, **kwargs):
        class _DynamicForm(RekapSuaraPilegProvForm):
            pass

        import copy
        _DynamicForm.base_fields = copy.deepcopy(RekapSuaraPilegProvForm.base_fields)
        _DynamicForm.declared_fields = copy.deepcopy(RekapSuaraPilegProvForm.declared_fields)
    ...
    def construct_change_message(self, request, form, formsets, add):
        return "Menambah rekap suara Pileg Provinsi baru." if add else "Memperbarui rincian perolehan suara (Partai & Caleg Provinsi)."
