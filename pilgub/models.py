...
def path_dan_nama_unik_gubernur(instance, filename):
    extension = filename.split('.')[-1]
    nama_baru = f"{instance.no_urut_paslon}_{uuid.uuid4().hex[:8]}.{extension}"
    return os.path.join('paslon_gubernur/', nama_baru)

class PaslonGubernur(models.Model):
    ...
    foto_paslon = models.ImageField(upload_to=path_dan_nama_unik_gubernur, null=True, blank=True, verbose_name="Foto Paslon")
    warna_hex = models.CharField(max_length=7, default='#808080', verbose_name="Warna")

    def __str__(self):
        return f"{self.no_urut_paslon}. {self.nama_cagub} - {self.nama_cawagub}"

    class Meta:
        verbose_name_plural = "Paslon Pilgub"
        ordering = ['no_urut_paslon']

class KoalisiGubernur(models.Model):
    paslon = models.ForeignKey(PaslonGubernur, on_delete=models.CASCADE, related_name='partai_pendukung', verbose_name="Paslon Gubernur")
    partai = models.OneToOneField(Partai, on_delete=models.CASCADE, verbose_name="Partai")

    def __str__(self):
        return f"{self.partai.nama_partai} -> {self.paslon.nama_cagub}"

    class Meta:
        verbose_name_plural = "Data Koalisi Gubernur"

class RekapSuaraGubernur(models.Model):
    kecamatan = models.OneToOneField(Kecamatan, on_delete=models.CASCADE, related_name='rekap_pilgub', verbose_name="Kecamatan")
    total_suara_tidak_sah = models.IntegerField(default=0, verbose_name="Total Suara Tidak Sah")

    @property
    def total_suara_sah(self):
        return sum(ds.jumlah_suara for ds in self.kecamatan.detail_pilgub.all())
    ...
    def get_suara_paslon(self, paslon):
        ...
        for ds in self.kecamatan.detail_pilgub.all():
...
class DetailSuaraGubernur(models.Model):
    kecamatan = models.ForeignKey(Kecamatan, on_delete=models.CASCADE, related_name='detail_pilgub', verbose_name="Kecamatan")
    paslon = models.ForeignKey(PaslonGubernur, on_delete=models.CASCADE, related_name='suara_wilayah', verbose_name="Paslon Gubernur")
...
class RekapKokabPilgubManager(models.Manager):
    def get_queryset(self):
        from django.db.models import Sum, Subquery, OuterRef, IntegerField
        from django.db.models.functions import Coalesce
        from core.models import Kecamatan
        from .models import DetailSuaraGubernur, RekapSuaraGubernur
        qs = super().get_queryset()
        return qs.annotate(
            agg_sah=Coalesce(
                Subquery(
                    DetailSuaraGubernur.objects.filter(kecamatan__kab_kota=OuterRef('pk'))
            ...
            agg_tidak_sah=Coalesce(
                Subquery(
                    RekapSuaraGubernur.objects.filter(kecamatan__kab_kota=OuterRef('pk'))
                    .values('kecamatan__kab_kota')
...
class RekapKokabPilgub(KabupatenKota):
    objects = RekapKokabPilgubManager()
    ...
    def get_total_suara_paslon(self, paslon):
        from django.db.models import Sum
        return DetailSuaraGubernur.objects.filter(
