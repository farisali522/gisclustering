...
def path_dan_nama_unik_caleg_kokab(instance, filename):
    extension = filename.split('.')[-1]
    # Simpan di folder foto_caleg_kokab/nama_dapil/nama_partai/
    ...
    return os.path.join(f'foto_caleg_kokab/{dapil_str}/{partai_str}/', nama_baru)

class DapilKokab(models.Model):
    nama_dapil = models.CharField(max_length=100, verbose_name="Nama Dapil")
    # 1 Kokab bisa punya banyak Dapil (ForeignKey)
    kab_kota = models.ForeignKey(KabupatenKota, on_delete=models.CASCADE, related_name='dapil_kokab', verbose_name="Kabupaten/Kota")
    jumlah_kursi = models.IntegerField(default=0, verbose_name="Jumlah Kursi")
    # 1 Dapil banyak Kecamatan, 1 Kecamatan bisa banyak Dapil (ManyToManyField)
    wilayah_kecamatan = models.ManyToManyField(Kecamatan, related_name='dapil_kokab', verbose_name="Cakupan Wilayah (Kecamatan)")
    
    def __str__(self):
        return f"{self.nama_dapil} - {self.kab_kota.nama_kokab}"

    class Meta:
        verbose_name_plural = "Dapil Kabupaten/Kota"
        ordering = ['kab_kota', 'nama_dapil']

class CalegKokab(models.Model):
    GENDER_CHOICES = (
        ('L', 'Laki-laki'),
        ('P', 'Perempuan'),
    )
    ...
    dapil = models.ForeignKey(DapilKokab, on_delete=models.CASCADE, related_name='caleg', verbose_name="Dapil")
    partai = models.ForeignKey(Partai, on_delete=models.CASCADE, related_name='caleg_kokab', verbose_name="Partai")
    jenis_kelamin = models.CharField(max_length=1, choices=GENDER_CHOICES, verbose_name="Jenis Kelamin", null=True, blank=True)
    foto_caleg = models.ImageField(upload_to=path_dan_nama_unik_caleg_kokab, null=True, blank=True, verbose_name="Foto Caleg (Opsional)")
...
class RekapSuaraPilegKokab(models.Model):
    kecamatan = models.OneToOneField(Kecamatan, on_delete=models.CASCADE, related_name='rekap_pileg_kokab', verbose_name="Kecamatan")
    total_suara_tidak_sah = models.IntegerField(default=0, verbose_name="Total Suara Tidak Sah")

    @property
    def total_suara_sah(self):
        return sum(d.jumlah_suara for d in self.kecamatan.detail_pileg_kokab.all())
    ...
    def get_total_suara_partai(self, partai):
        ...
        """
        return sum(d.jumlah_suara for d in self.kecamatan.detail_pileg_kokab.all() if d.partai_id == partai.id)
...
class DetailSuaraPilegKokab(models.Model):
    kecamatan = models.ForeignKey(Kecamatan, on_delete=models.CASCADE, related_name='detail_pileg_kokab', verbose_name="Kecamatan")
    partai = models.ForeignKey(Partai, on_delete=models.CASCADE, related_name='suara_pileg_kokab', verbose_name="Partai")
    caleg = models.ForeignKey(CalegKokab, on_delete=models.CASCADE, related_name='suara_pileg_kokab', verbose_name="Caleg", null=True, blank=True)
...
class RekapSuaraPilegKokabKabManager(models.Manager):
    def get_queryset(self):
        from django.db.models import Sum, Subquery, OuterRef, IntegerField
        from django.db.models.functions import Coalesce
        from core.models import Kecamatan
        from .models import DetailSuaraPilegKokab, RekapSuaraPilegKokab
        qs = super().get_queryset()
        return qs.annotate(
            agg_sah=Coalesce(
                Subquery(
                    DetailSuaraPilegKokab.objects.filter(kecamatan__kab_kota=OuterRef('pk'))
            ...
            agg_tidak_sah=Coalesce(
                Subquery(
                    RekapSuaraPilegKokab.objects.filter(kecamatan__kab_kota=OuterRef('pk'))
                    .values('kecamatan__kab_kota')
...
class RekapSuaraPilegKokabKab(KabupatenKota):
    objects = RekapSuaraPilegKokabKabManager()
    ...
    def get_suara_partai(self, partai_id):
        ...
        return DetailSuaraPilegKokab.objects.filter(
            kecamatan__kab_kota=self, partai_id=partai_id
        ).aggregate(total=Sum('jumlah_suara'))['total'] or 0

class RekapSuaraPilegKokabDapilManager(models.Manager):
    def get_queryset(self):
        from django.db.models import Sum, Subquery, OuterRef, IntegerField
        from django.db.models.functions import Coalesce
        from core.models import Kecamatan
        ...
        return qs.annotate(
            agg_sah=Coalesce(
                Subquery(
                    DetailSuaraPilegKokab.objects.filter(
                        kecamatan__dapil_kokab=OuterRef('pk')
                    )
                    .values('kecamatan__dapil_kokab')
                    .annotate(t=Sum('jumlah_suara'))
                    .values('t'),
            ...
            agg_tidak_sah=Coalesce(
                Subquery(
                    RekapSuaraPilegKokab.objects.filter(
                        kecamatan__dapil_kokab=OuterRef('pk')
                    )
                    .values('kecamatan__dapil_kokab')
                    .annotate(t=Sum('total_suara_tidak_sah'))
...
class RekapSuaraPilegKokabDapil(DapilKokab):
    objects = RekapSuaraPilegKokabDapilManager()
    ...
    def get_suara_partai(self, partai_id):
        from django.db.models import Sum
        return DetailSuaraPilegKokab.objects.filter(
            kecamatan__dapil_kokab=self, partai_id=partai_id
        ).aggregate(total=Sum('jumlah_suara'))['total'] or 0