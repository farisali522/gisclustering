...
def path_dan_nama_unik_caleg_ri(instance, filename):
    extension = filename.split('.')[-1]
    # Simpan di folder foto_caleg_ri/nama_dapil/nama_partai/
    dapil_str = instance.dapil.nama_dapil if instance.dapil else "tampa_dapil"
    ...
    return os.path.join(f'foto_caleg_ri/{dapil_str}/{partai_str}/', nama_baru)
...
class DapilRI(models.Model):
    ...
    wilayah_kokab = models.ManyToManyField(KabupatenKota, related_name='dapil_ri', verbose_name="Cakupan Wilayah (Kab/Kota)")
...
class CalegRI(models.Model):
    ...
    dapil = models.ForeignKey(DapilRI, on_delete=models.CASCADE, related_name='caleg', verbose_name="Dapil")
    partai = models.ForeignKey(Partai, on_delete=models.CASCADE, related_name='caleg', verbose_name="Partai")
    jenis_kelamin = models.CharField(max_length=1, choices=GENDER_CHOICES, verbose_name="Jenis Kelamin", null=True, blank=True)
    foto_caleg = models.ImageField(upload_to=path_dan_nama_unik_caleg_ri, null=True, blank=True, verbose_name="Foto Caleg (Opsional)")
...
class RekapSuaraPilegRI(models.Model):
    kecamatan = models.OneToOneField(Kecamatan, on_delete=models.CASCADE, related_name='rekap_pileg_ri', verbose_name="Kecamatan")
    total_suara_tidak_sah = models.IntegerField(default=0, verbose_name="Total Suara Tidak Sah")

    @property
    def total_suara_sah(self):
        return sum(d.jumlah_suara for d in self.kecamatan.detail_pileg_ri.all())
    ...
    def get_total_suara_partai(self, partai):
        ...
        """
        return sum(d.jumlah_suara for d in self.kecamatan.detail_pileg_ri.all() if d.partai_id == partai.id)
...
class DetailSuaraPilegRI(models.Model):
    kecamatan = models.ForeignKey(Kecamatan, on_delete=models.CASCADE, related_name='detail_pileg_ri', verbose_name="Kecamatan")
    partai = models.ForeignKey(Partai, on_delete=models.CASCADE, related_name='suara_pileg_ri', verbose_name="Partai")
    caleg = models.ForeignKey(CalegRI, on_delete=models.CASCADE, related_name='suara_pileg_ri', verbose_name="Caleg", null=True, blank=True)
...
class RekapSuaraPilegRIKabManager(models.Manager):
    def get_queryset(self):
        from django.db.models import Sum, Subquery, OuterRef, IntegerField
        from django.db.models.functions import Coalesce
        from core.models import Kecamatan
        from .models import DetailSuaraPilegRI, RekapSuaraPilegRI
        qs = super().get_queryset()
        return qs.annotate(
            agg_sah=Coalesce(
                Subquery(
                    DetailSuaraPilegRI.objects.filter(kecamatan__kab_kota=OuterRef('pk'))
            ...
            agg_tidak_sah=Coalesce(
                Subquery(
                    RekapSuaraPilegRI.objects.filter(kecamatan__kab_kota=OuterRef('pk'))
                    .values('kecamatan__kab_kota')
...
class RekapSuaraPilegRIKab(KabupatenKota):
    objects = RekapSuaraPilegRIKabManager()
    ...
    def get_suara_partai(self, partai_id):
        ...
        return DetailSuaraPilegRI.objects.filter(
            kecamatan__kab_kota=self, partai_id=partai_id
        ).aggregate(total=Sum('jumlah_suara'))['total'] or 0

class RekapSuaraPilegRIDapilManager(models.Manager):
    def get_queryset(self):
        from django.db.models import Sum, Subquery, OuterRef, IntegerField
        from django.db.models.functions import Coalesce
        from core.models import Kecamatan
        ...
        return qs.annotate(
            agg_sah=Coalesce(
                Subquery(
                    DetailSuaraPilegRI.objects.filter(
                        kecamatan__kab_kota__dapil_ri=OuterRef('pk')
                    )
                    .values('kecamatan__kab_kota__dapil_ri')
                    .annotate(t=Sum('jumlah_suara'))
                    .values('t'),
            ...
            agg_tidak_sah=Coalesce(
                Subquery(
                    RekapSuaraPilegRI.objects.filter(
                        kecamatan__kab_kota__dapil_ri=OuterRef('pk')
                    )
                    .values('kecamatan__kab_kota__dapil_ri')
                    .annotate(t=Sum('total_suara_tidak_sah'))
            ...
            agg_tps=Coalesce(
                Subquery(
                    Kecamatan.objects.filter(
                        kab_kota__dapil_ri=OuterRef('pk')
                    )
                    .values('kab_kota__dapil_ri')
                    .annotate(t=Sum('tps_pemilu__rekap_tps_pemilu'))
                    .values('t'),
                    output_field=IntegerField()
                ),
...
class RekapSuaraPilegRIDapil(DapilRI):
    objects = RekapSuaraPilegRIDapilManager()
    ...
    def get_suara_partai(self, partai_id):
        from django.db.models import Sum
        return DetailSuaraPilegRI.objects.filter(
            kecamatan__kab_kota__dapil_ri=self, partai_id=partai_id
        ).aggregate(total=Sum('jumlah_suara'))['total'] or 0