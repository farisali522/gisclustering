...
from .models import DapilRI, CalegRI, RekapSuaraPilegRI, DetailSuaraPilegRI, RekapSuaraPilegRIKab, RekapSuaraPilegRIDapil

@admin.register(RekapSuaraPilegRIKab)
class RekapSuaraPilegRIKabAdmin(admin.ModelAdmin):
    list_display = ('wilayah_kab', 'info_tps_dpt_kab')
    list_filter = ('dapil_ri',)
        ...
        for p in partais:
            ...
            def _gv_kab(obj, pid=p_id, c=p_color):
                if not hasattr(obj, '_party_votes'):
                    ...
                    qs = DetailSuaraPilegRI.objects.filter(kecamatan__kab_kota=obj).values('partai_id').annotate(total=Sum('jumlah_suara'))
    ...
    def wilayah_kab(self, obj):
        ...
        url = reverse('admin:pileg_ri_rekapsuarapilegri_changelist')
        full_url = f"{url}?kecamatan__kab_kota__id__exact={obj.id}"
...
@admin.register(RekapSuaraPilegRIDapil)
class RekapSuaraPilegRIDapilAdmin(admin.ModelAdmin):
    list_display = ('wilayah_dapil', 'info_tps_dpt_dapil')
    search_fields = ('nama_dapil',)
    ordering = ('nama_dapil',)
    ...
    def get_list_display(self, request):
        ...
        for p in partais:
            ...
            def _gv_dapil(obj, pid=p_id, c=p_color):
                if not hasattr(obj, '_party_votes'):
                    from django.db.models import Sum
                    qs = DetailSuaraPilegRI.objects.filter(kecamatan__kab_kota__dapil_ri=obj).values('partai_id').annotate(total=Sum('jumlah_suara'))
                    obj._party_votes = {item['partai_id']: item['total'] for item in qs}
                    obj._max_vote = max(obj._party_votes.values()) if obj._party_votes else -1
    ...
    def wilayah_dapil(self, obj):
        from django.urls import reverse
        cakupan = ", ".join([k.nama_kokab for k in obj.wilayah_kokab.all()]) or "-"
        url = reverse('admin:pileg_ri_rekapsuarapilegrikab_changelist')
        full_url = f"{url}?dapil_ri__id__exact={obj.id}"
        return format_html('<div style="line-height:1.2;"><a href="{}" title="Buka Detail Kabupaten untuk Dapil Ini"><b>{}</b></a><br><span style="font-size:10.5px; color:#666;">{}</span></div>', full_url, obj.nama_dapil, cakupan)
...
class DapilRIAdmin(admin.ModelAdmin):
    list_display = ('nama_dapil', 'jumlah_kursi', 'wilayah_cakupan')
    search_fields = ('nama_dapil',)
    filter_horizontal = ('wilayah_kokab',)

    def wilayah_cakupan(self, obj):
        nama_kolom = [k.nama_kokab for k in obj.wilayah_kokab.all()]
        return ", ".join(nama_kolom) if nama_kolom else "-"
    wilayah_cakupan.short_description = "Cakupan Wilayah"

class CalegRIResource(resources.ModelResource):
    dapil = fields.Field(column_name='dapil', attribute='dapil', widget=ForeignKeyWidget(DapilRI, 'nama_dapil'))
...
class CalegRIAdmin(ImportExportModelAdmin):
    ...
    def identitas_caleg(self, obj):
        ...
        # Desain Avatar Inisial Elegan (Pola ala Google/Slack)
        if obj.nama_caleg:
            kata = obj.nama_caleg.strip().split()
            if len(kata) >= 2:
                ikon = (kata[0][0] + kata[1][0]).upper()  # Ambil 2 huruf dari nama depan & tengah
            elif len(kata) == 1:
        ...
        # Palet warna premium ala desain modern
        if obj.jenis_kelamin == 'P':
            warna_bg = '#ff7675' # Pink lembut (soft coral)
            warna_tx = '#ffffff'
        else:
            warna_bg = '#74b9ff' # Biru langit elegan
            warna_tx = '#ffffff'
...
class RekapSuaraPilegRIResource(resources.ModelResource):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            # Susun berjejer: [Suara Partai] -> [Suara Caleg1] -> [Suara Caleg2] -> dst per partai
            partais = Partai.objects.all().order_by('no_urut_partai')
            for p in partais:
                # 1. Suntikkan Header Kolom "Coblos Logo Partai Saja"
                fn_p = f"P_{p.nama_partai}"
                if fn_p not in self.fields:
                    self.fields[fn_p] = fields.Field(column_name=fn_p, attribute=fn_p)
                
                # 2. Lacak seluruh caleg di partai tersebut, lalu langsung dijejerkan di taruh di sebelahnya
                calegs_prt = CalegRI.objects.filter(partai=p).order_by('no_urut')
                for c in calegs_prt:
                    fn_c = f"C_{p.nama_partai}_{c.no_urut}"
    ...
    def get_export_order(self):
        # Override native method agar 100% selalu mengikuti urutan keys yang baru saja kita rakit
        return list(self.fields.keys())

    def export_field(self, field, obj, **kwargs):
        # Saat export, cari nilainya dari DetailSuaraPilegRI yang sesuai
        if field.attribute and field.attribute.startswith('P_'):
            nama_p = field.attribute[2:]
            ds = DetailSuaraPilegRI.objects.filter(kecamatan=obj.kecamatan, partai__nama_partai__iexact=nama_p, caleg__isnull=True).first()
            ...
            if len(parts) == 2 and parts[1].isdigit():
                ...
                ds = DetailSuaraPilegRI.objects.filter(kecamatan=obj.kecamatan, partai__nama_partai__iexact=nama_p, caleg__no_urut=no_urut).first()
                return ds.jumlah_suara if ds else 0
    ...
    def after_import_row(self, row, row_result, **kwargs):
        ...
        # === KUNCI FIX: Cari Dapil dari Kecamatan ini ===
        try:
            dapil_kec = kec.kab_kota.dapil_ri.first()
        except Exception:
            dapil_kec = None
        ...
        for key, val in row.items():
            ...
            # Format C_NAMA_NO (Misal: C_GERINDRA_1)
                ...
                if len(parts) == 2 and parts[1].isdigit():
                    ...
                    prt = Partai.objects.filter(nama_partai__iexact=nama_p).first()
                    if prt:
                        # === FIX: Filter caleg berdasarkan Dapil Kecamatan! ===
                        if dapil_kec:
                            clg = CalegRI.objects.filter(partai=prt, no_urut=no_urut, dapil=dapil_kec).first()
                        else:
                            clg = None
                        if clg:
                            DetailSuaraPilegRI.objects.update_or_create(
...
class RekapSuaraPilegRIForm(forms.ModelForm):
    class Meta:
        model = RekapSuaraPilegRI
        fields = ['kecamatan', 'total_suara_tidak_sah']
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Simpan filter URL ke hidden field agar tidak hilang saat Save
        ...
        # Cari Dapil dari Kecamatan
        try:
            dapil_kec = obj.kecamatan.kab_kota.dapil_ri.first()
            nama_kab  = obj.kecamatan.kab_kota.nama_kokab
            nama_dapil = dapil_kec.nama_dapil if dapil_kec else "-"
        except Exception:
            dapil_kec = None
            nama_kab  = "-"
            nama_dapil = "-"
        ...
        #rekapsuarapilegri_form, #content-main {
        ...
        # === INFO BOX (seperti siapa) ===
        self.fields['info_kb'] = forms.CharField(
            label=mark_safe(script_hitung + "Kabupaten/Kota"), initial=nama_kab,
            required=False, disabled=True, widget=forms.TextInput(attrs={'class': 'readonly'}))
        self.fields['info_dp'] = forms.CharField(
            label="Dapil RI", initial=nama_dapil,
            required=False, disabled=True, widget=forms.TextInput(attrs={'class': 'readonly'}))
        self.fields['res_s'] = forms.CharField(
            label="Total Suara Sah", initial=fmt(t_sah),
            required=False, disabled=True,
            widget=forms.TextInput(attrs={'class': 'readonly', 'style': 'color:#28a745;'}))
        self.fields['res_t'] = forms.CharField(
            label="Total Suara Masuk", initial=fmt(t_total),
            required=False, disabled=True,
            widget=forms.TextInput(attrs={'class': 'readonly', 'style': 'color:#007bff;'}))
        ...
        # Tampilkan banner reset filter kalau sedang filter partai
        if filter_p:
            from django.urls import reverse
            reset_url = reverse('admin:pileg_ri_rekapsuarapilegri_change', args=[obj.pk])
            self.fields['reset_filter'] = forms.CharField(
                label=mark_safe(
                    f'<div style="display:inline-flex; align-items:center;">'
                    f'<a href="{reset_url}" style="background:#ffc107; color:#212529; text-decoration:none; padding:6px 14px; border-radius:4px; font-weight:600; font-size:13px; box-shadow:0 1px 2px rgba(0,0,0,0.1);">Tampilkan Semua Partai</a>'
                    f'</div>'),
                required=False, disabled=True, widget=forms.HiddenInput())
            order['reset_filter'] = self.fields.pop('reset_filter')

        # Sembunyikan info box saat filter partai (ala siapa)
        ...
        # === PREFETCH DATA SUARA ===
        p_ids = {d.partai_id: d.jumlah_suara for d in DetailSuaraPilegRI.objects.filter(kecamatan=obj.kecamatan, caleg__isnull=True)}
        c_ids = {d.caleg_id: d.jumlah_suara for d in DetailSuaraPilegRI.objects.filter(kecamatan=obj.kecamatan, caleg__isnull=False)}

        caleg_by_partai = defaultdict(list)
        if dapil_kec:
            for c in CalegRI.objects.filter(dapil=dapil_kec).select_related('partai').order_by('no_urut'):
                caleg_by_partai[c.partai_id].append(c)

        # Total suara partai yang difilter (ala siapa)
        if filter_p:
            ...
            if p_obj:
                ...
                self.fields['res_p_total'] = forms.CharField(
                    label=mark_safe(f"{logo_html}<b>Total Suara {p_obj.nama_partai} (Partai + Caleg)</b>"),
                    initial=fmt(p_suara + c_suara_total),
                    required=False, disabled=True,
                    widget=forms.TextInput(attrs={'class': 'readonly', 'style': 'color:#e83e8c; background:#fff0f5; border:2px solid #e83e8c;'}))
                order['res_p_total'] = self.fields.pop('res_p_total')

        if edit_ts:
            from django.urls import reverse
            reset_url = reverse('admin:pileg_ri_rekapsuarapilegri_change', args=[obj.pk])
            self.fields['reset_filter_ts'] = forms.CharField(
                label=mark_safe(
                    f'<div style="display:inline-flex; align-items:center;">'
                    f'<a href="{reset_url}" style="background:#f8f9fa; color:#495057; border:1px solid #ced4da; text-decoration:none; padding:6px 14px; border-radius:4px; font-weight:600; font-size:13px;">Tampilkan Seluruh Partai & Caleg &rarr;</a>'
                    f'</div>'),
                required=False, disabled=True, widget=forms.HiddenInput())
        ...
        # === DAFTAR SUARA PARTAI + CALEG ===
        partais = Partai.objects.all().order_by('no_urut_partai')
        for p in partais:
            # Filter partai jika ada filter_p
            ...
            f_p = f'su_p_{p.id}'
            self.fields[f_p] = forms.IntegerField(
                label=mark_safe(f"{logo}<b>{p.no_urut_partai}. {p.nama_partai}</b>"),
                initial=val_p, required=False, min_value=0,
                help_text=format_html('<span style="color:#666;">{} dari Suara Sah</span>', pct_p),
                widget=forms.NumberInput(attrs={'class': 'input-paslon', 'style': 'font-weight:bold; border-color:#333;'}))
            ...
            for c in caleg_by_partai.get(p.id, []):
                ...
                f_c = f'su_c_{c.id}'
                self.fields[f_c] = forms.IntegerField(
                    label=f"\u2514 {c.no_urut}. {c.nama_caleg}",
                    initial=val_c, required=False, min_value=0,
                    help_text=format_html('<span style="color:#666;">{} dari Suara Sah</span>', pct_c),
                    widget=forms.NumberInput(attrs={'class': 'input-paslon'}))
                order[f_c] = self.fields.pop(f_c)

        self.fields = order


@admin.register(RekapSuaraPilegRI)
class RekapSuaraPilegRIAdmin(ImportExportModelAdmin):
    form = RekapSuaraPilegRIForm
    resource_class = RekapSuaraPilegRIResource
    formats = [XLSX]
    ...
    def get_list_display(self, request):
        ...
        # Hapus p & f dari query params untuk link partai
        ...
        # Kolom per Partai (bisa diklik → filter form)
        partais = list(Partai.objects.all().order_by('no_urut_partai'))
        for p in partais:
            fn = f'col_p_{p.id}'
            p_id = p.id
            ...
            def _gv(obj, pid=p_id, c=p_color, bq=p_query, qc=p_conn):
                ...
                if not hasattr(obj, '_party_votes'):
                    from django.db.models import Sum
                    qs = DetailSuaraPilegRI.objects.filter(kecamatan=obj.kecamatan).values('partai_id').annotate(total=Sum('jumlah_suara'))
                    obj._party_votes = {item['partai_id']: item['total'] for item in qs}
                    obj._max_vote = max(obj._party_votes.values()) if obj._party_votes else -1
            ...
            if p_logo:
                _gv.short_description = mark_safe(
                    ...
                    f'<img src="{p_logo}" title="{p_name}" style="height:20px; object-fit:contain;"><br>'
                    f'<span style="font-size:11px;">#{p_no}</span></div>')
            else:
    ...
    def save_model(self, request, obj, form, change):
        ...
        for n, v in form.cleaned_data.items():
            if n.startswith('su_p_'):
                ...
                clg = CalegRI.objects.get(id=int(n[5:]))
                DetailSuaraPilegRI.objects.update_or_create(
    ...
    def get_fields(self, request, obj=None):
        if obj and obj.pk:
            dummy = RekapSuaraPilegRIForm(instance=obj, request=request)
            return list(dummy.fields.keys())
        return ['kecamatan', 'total_suara_tidak_sah']

    def get_form(self, request, obj=None, **kwargs):
        class _DynamicForm(RekapSuaraPilegRIForm):
            pass

        import copy
        _DynamicForm.base_fields = copy.deepcopy(RekapSuaraPilegRIForm.base_fields)
        _DynamicForm.declared_fields = copy.deepcopy(RekapSuaraPilegRIForm.declared_fields)
    ...
    def construct_change_message(self, request, form, formsets, add):
        return "Menambah rekap suara baru." if add else "Memperbarui rincian perolehan suara (Partai & Caleg)."
